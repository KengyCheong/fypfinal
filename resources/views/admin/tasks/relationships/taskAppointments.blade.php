...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-taskAppointments">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.appointment.fields.approval_status') }}
                        </th>
                        <th>
                            {{ trans('cruds.appointment.fields.task') }}
                        </th>
                        <th>
                            {{ trans('cruds.appointment.fields.appointment_time_date') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($appointments as $key => $appointment)
                        <tr data-entry-id="{{ $appointment->id }}">
                            ...
                            <td>
                                {{ App\Models\Appointment::APPROVAL_STATUS_SELECT[$appointment->approval_status] ?? '' }}
                            </td>
                            <td>
                                @foreach($appointment->tasks as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ $appointment->appointment_time_date ?? '' }}
                            </td>
                            <td>
...
@endcan
  ...
  let table = $('.datatable-taskAppointments:not(.ajaxTable)').DataTable({ buttons: dtButtons })
