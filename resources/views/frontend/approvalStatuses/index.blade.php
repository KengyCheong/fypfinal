        ...
        <div class="col-md-12">
            @can('approval_status_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.approval-statuses.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $approvalStatus->id }}">
                                        ...
                                        <td>
                                            @can('approval_status_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.approval-statuses.show', $approvalStatus->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('approval_status_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.approval-statuses.edit', $approvalStatus->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('approval_status_delete')
                                                <form action="{{ route('frontend.approval-statuses.destroy', $approvalStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('approval_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.approval-statuses.massDestroy') }}",
