        ...
        <div class="col-md-12">
            @can('vaccine_tag_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.vaccine-tags.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $vaccineTag->id }}">
                                        ...
                                        <td>
                                            @can('vaccine_tag_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.vaccine-tags.show', $vaccineTag->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('vaccine_tag_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.vaccine-tags.edit', $vaccineTag->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('vaccine_tag_delete')
                                                <form action="{{ route('frontend.vaccine-tags.destroy', $vaccineTag->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('vaccine_tag_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.vaccine-tags.massDestroy') }}",
