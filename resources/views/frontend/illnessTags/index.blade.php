        ...
        <div class="col-md-12">
            @can('illness_tag_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.illness-tags.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $illnessTag->id }}">
                                        ...
                                        <td>
                                            @can('illness_tag_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.illness-tags.show', $illnessTag->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('illness_tag_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.illness-tags.edit', $illnessTag->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('illness_tag_delete')
                                                <form action="{{ route('frontend.illness-tags.destroy', $illnessTag->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('illness_tag_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.illness-tags.massDestroy') }}",
